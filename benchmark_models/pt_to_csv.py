...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert .pt results from csv",
...
def pt_to_csv(
    ...
    if input_folder_path is None:
        input_folder_path = (
            f"output/{network_name}/pt/{faulty_clean_str}/batch_size_{batch_size}"
        )

    # Initialize (and create) the output folder path
    output_folder_path = (
        f"output/{network_name}/csv/{faulty_clean_str}/batch_size_{batch_size}"
    )
    ...
    with open(output_file_path, "w") as output_file:
        ...
        for file in pbar:
            ...
            if file.endswith(".pt"):
                # Open the input file
                tensor = torch.load(
                    f"{input_folder_path}/{file}", map_location=torch.device("cpu")
                )  # This avoids loading data to GPU
                if not label:
    ...
    # Prepare the header
    if not label:
        header_list = [f"score_class_{i}" for i in range(number_of_classes)] + [
            "batch_id",
            "image_id",
        ]
